...
def dhcp(cgx):
    ...
    for site in cgx.get.sites().cgx_content['items']:
        for dhcp in cgx.get.dhcpservers(site_id=site["id"]).cgx_content['items']:
            dhcp["site_name"] = site["name"]
            dhcp.pop("_etag")
            dhcp.pop("_schema")
            dhcp.pop("_created_on_utc")
            dhcp.pop("_updated_on_utc")
            dhcp.pop("_debug")
            dhcp.pop("_info")
            dhcp.pop("_warning")
            dhcp.pop("_error")
            dhcp_list.append(dhcp)
            
    
    csv_columns = []
    for key in dhcp_list[0]:
        csv_columns.append(key)
    csv_file = "dhcp_list.csv"
    try:
        with open(csv_file, 'w', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
            writer.writeheader()
            for data in dhcp_list:
                try:
                    writer.writerow(data)
                except:
                    print("Failed to write data for row")
            print("\nSaved dhcp_list.csv file")
    except IOError:
